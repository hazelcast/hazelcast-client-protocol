...
type ${model.parentName}${model.name?cap_first}ResponseParameters struct {
	...
	${p.name?cap_first} ${util.getGoType(p.type)} 
	</#list>
}
<#--************************ REQUEST ********************************************************-->

func ${model.parentName}${model.name?cap_first}CalculateSize(<#list model.requestParams as param>${param.name} <#if param.nullable>*</#if>${util.getGoType(param.type)}  <#if param_has_next>, </#if></#list>) int {
...
func ${model.parentName}${model.name?cap_first}EncodeRequest(<#list model.requestParams as param>${param.name} <#if param.nullable>*</#if>${util.getGoType(param.type)} <#if param_has_next>, </#if></#list>) *ClientMessage {
...
func ${model.parentName}${model.name?cap_first}Handle(clientMessage *ClientMessage, <#list model.events as event>handleEvent${event.name?cap_first} func(<#list event.eventParams as p>${util.getGoType(p.type)}<#if p_has_next>,</#if></#list>)<#if event_has_next>, </#if></#list>){
    // Event handler
...
<#--SIZE MACRO -->
<#macro sizeTextInternal var_name type indent isNullable=false>
...
${""?left_pad(indent * 4)}dataSize += ${util.getGoType(type)?upper_case}_SIZE_IN_BYTES
        <#else >
${""?left_pad(indent * 4)}dataSize += ${util.getGoType(type)?cap_first}CalculateSize(<#if isNullable==false>&</#if>${var_name})
        </#if>
        <#break >
    <#case "CUSTOM">
${""?left_pad(indent * 4)}dataSize += ${util.getGoType(type)?cap_first}CalculateSize(<#if isNullable==false>&</#if>${var_name})
...
${""?left_pad(indent * 4)}dataSize += INT_SIZE_IN_BYTES
        ...
        <#local n= var_name>
${""?left_pad(indent * 4)}for _,${var_name}Item := range <#if isNullable>*</#if>${var_name}{
...
${""?left_pad(indent * 4)}dataSize += INT_SIZE_IN_BYTES
        ...
        <#local n= var_name>
${""?left_pad(indent * 4)}for range <#if isNullable>*</#if>${var_name}{
        <@sizeTextInternal var_name="${n}Item"  type=genericType indent=(indent + 1)/>
        }
        <#break >
    <#case "MAP">
        ...
        <#local n= var_name>
${""?left_pad(indent * 4)}for key, val := range <#if isNullable>*</#if>${var_name}{
    ...
    <#case "MAPENTRY">
        <#local keyType = util.getFirstGenericParameterType(type)>
        <#local valueType = util.getSecondGenericParameterType(type)>
        key := ${var_name}.Key.(${util.getGoType(keyType)})
        val := ${var_name}.Value.(${util.getGoType(valueType)})
...
<#--SETTER MACRO -->
<#macro setterTextInternal var_name type indent isNullable=false>
    <#local cat= util.getTypeCategory(type)>
    <#if cat == "OTHER">
${""?left_pad(indent * 4)}clientMessage.Append${util.getGoType(type)?cap_first}(<#if isNullable>*</#if>${var_name})
    </#if>
    <#if cat == "CUSTOM">
${""?left_pad(indent * 4)}${util.getTypeCodec(type)?split(".")?last}Encode(clientMessage, &${var_name})
    </#if>
    <#if cat == "COLLECTION">
${""?left_pad(indent * 4)}clientMessage.AppendInt(len(<#if isNullable>*</#if>${var_name}))
        <#local itemType = util.getGenericType(type)>
        <#local itemTypeVar= var_name + "Item">
${""?left_pad(indent * 4)}for _,${itemTypeVar} := range <#if isNullable>*</#if>${var_name}{
...
<#if isNullable>
<#if (isEvent)>${""?left_pad(indent * 4)}var ${var_name} ${util.modifyForGoTypes(type?split(".")?last)}</#if>
...
${""?left_pad(indent * 4)}<#if !(isEvent || isCollection)>parameters.${var_name?cap_first}<#else>${var_name} <#if isNullable==false>:</#if></#if>= clientMessage.Read${util.getGoType(varType)?cap_first}()
        ...
        <#break >
    <#case "CUSTOM">
${""?left_pad(indent * 4)}<#if !(isEvent || isCollection)>parameters.${var_name?cap_first}<#else>${var_name} <#if isNullable==false>:</#if></#if>= *${util.getTypeCodec(varType)?split(".")?last}Decode(clientMessage)
        <#break >
    <#case "COLLECTION">
    <#case "ARRAY">
    <#if cat == "COLLECTION">
...
${""?left_pad(indent * 4)}    ${var_name} = append(${var_name},${itemVariableName})
${""?left_pad(indent * 4)}}
...
${""?left_pad(indent * 4)}<#if !isEvent>parameters['${var_name}'] = ${var_name}</#if>
    <#case "MAPENTRY">
        ...
        <#local keyVariableName= "${var_name}Key">
        <#local valVariableName= "${var_name}Val">
${""?left_pad(indent * 4)}var ${var_name} <#if varType?contains("Map.Entry")>Pair<#else>${util.modifyForGoTypes(itemVariableType?split(".")?last)}</#if>
